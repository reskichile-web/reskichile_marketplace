...
import type { AdminUsersPageData } from '@/lib/admin-view-data'
...
export default function AdminUsersClient({ initialData }: { initialData: AdminUsersPageData }) {
  const [users, setUsers] = useState<UserWithProducts[]>(initialData.users)
  ...
  const [currentUserId, setCurrentUserId] = useState<string | null>(initialData.currentUserId)
  ...
  useEffect(() => {
    if (initialRenderRef.current) {
      ...
      void hydrateEmailHealth(initialData.users)
      return
    }
    setExpandedId(null)
    void load(0)
  }, [hydrateEmailHealth, initialData.users, load])
