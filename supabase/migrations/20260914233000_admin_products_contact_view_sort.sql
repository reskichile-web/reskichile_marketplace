-- Engagement ordering is intentionally compound: completed contacts are the
-- primary signal and product views only break ties. Both are calculated before
-- pagination so every page preserves the same global order.
BEGIN;
...
DECLARE
  ...
  v_brands TEXT[] := ARRAY(
    ...
    FROM unnest(string_to_array(coalesce(p_brand, ''), E'\n')) AS value
    WHERE btrim(value) <> ''
  );
  v_product_types TEXT[] := ARRAY(
    SELECT DISTINCT btrim(value)
    FROM unnest(string_to_array(coalesce(p_product_type, ''), E'\n')) AS value
    WHERE btrim(value) <> ''
  );
  v_search TEXT := lower(coalesce(btrim(p_search), ''));
  v_product_sort TEXT := CASE
    WHEN lower(coalesce(btrim(p_time_sort), '')) IN ('asc', 'desc', 'views_asc', 'views_desc')
  ...
  RETURN (
    ...
    filtered AS (
      ...
      WHERE (v_status = 'all' OR status = v_status)
        AND (cardinality(v_brands) = 0 OR brand = ANY(v_brands))
        AND (cardinality(v_product_types) = 0 OR product_type = ANY(v_product_types))
        AND (
          v_search = ''
    ...
    ranked_rows AS (
      SELECT
        filtered.*,
        CASE WHEN v_product_sort IN ('views_asc', 'views_desc') THEN (
          SELECT count(*)::BIGINT
          FROM public.events AS event
          WHERE event.product_id = filtered.id
            AND event.event_type = 'click'
            AND event.event_name IN ('whatsapp_contact', 'chat_contact')
        ) ELSE NULL END AS sort_contact_count,
        CASE WHEN v_product_sort IN ('views_asc', 'views_desc') THEN (
          SELECT count(*)::BIGINT
          FROM public.events AS event
          WHERE event.product_id = filtered.id
            AND event.event_type = 'product_view'
        ) ELSE NULL END AS sort_view_count
      FROM filtered
    ),
    page_rows AS (
      SELECT
        ...
        row_number() OVER (
          ORDER BY
            CASE WHEN v_product_sort = 'views_desc' THEN sort_contact_count END DESC,
            CASE WHEN v_product_sort = 'views_desc' THEN sort_view_count END DESC,
            CASE WHEN v_product_sort = 'views_asc' THEN sort_contact_count END ASC,
            CASE WHEN v_product_sort = 'views_asc' THEN sort_view_count END ASC,
            CASE WHEN v_product_sort = 'asc' THEN days_published END ASC,
            CASE WHEN v_product_sort = 'desc' THEN days_published END DESC,
            CASE WHEN v_product_sort = 'desc' THEN created_at END ASC,
            created_at DESC,
            id
        ) AS sort_position
    ...
    enriched_rows AS (
      SELECT
        ...
        coalesce(page.sort_view_count, views.value, 0)::BIGINT AS view_count
      ...
      LEFT JOIN LATERAL (
        ...
        WHERE event.product_id = page.id
          AND event.event_type = 'product_view'
      ) AS views ON page.sort_view_count IS NULL
    ),
    status_groups AS (
