 ...
 *
 * Uso:
 *   1. Coloca el archivo Excel en scripts/productos.xlsx
 *   2. Crea scripts/.env con SUPABASE_URL y SUPABASE_SERVICE_ROLE_KEY
 *   3. node scripts/import-products.mjs
 */
...
try {
  ...
  console.error('No se encontró .env.local — asegúrate de que exista')
  process.exit(1)
}
...
async function main() {
  ...
  try {
    ...
    console.error('   Coloca el archivo Excel en esa ruta e intenta de nuevo.')
    process.exit(1)
  }
  ...
  // Cache de usuarios ya creados en esta sesión
  const userCache = {}
  ...
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i]
    const rowNum = i + 2 // fila real en Excel (header = 1)
    ...
    if (!email) { console.warn(`⚠️  Fila ${rowNum}: sin email, saltando`); skipped++; continue }
    if (!productTypeRaw) { console.warn(`⚠️  Fila ${rowNum}: sin tipo de producto, saltando`); skipped++; continue }
    if (!price || price <= 0) { console.warn(`⚠️  Fila ${rowNum}: precio inválido (${priceRaw}), saltando`); skipped++; continue }

    const productType = mapProductType(productTypeRaw)
    if (!productType) { console.warn(`⚠️  Fila ${rowNum}: tipo desconocido "${productTypeRaw}", saltando`); skipped++; continue }

    // ── 1. Obtener o crear usuario ──
    let userId = userCache[email]

    if (!userId) {
      // Busca si ya existe en auth
      ...
      if (existing) {
        ...
        // Crea usuario en auth con contraseña temporal
    ...
    // ── 2. Extraer campos del producto ──
    const fields = extractProductFields(productType, row)
    if (!fields) { console.warn(`⚠️  Fila ${rowNum}: no se pudieron extraer campos`); skipped++; continue }
    ...
    if (productError) {
      console.error(`❌ Fila ${rowNum}: error insertando producto: ${productError.message}`)
      errors++
    } else {
      console.log(`✅ Fila ${rowNum}: ${productType} - ${fields.brand} ${fields.model || ''} ($${price?.toLocaleString('es-CL')})`)
      created++
    }
  }
